...
public protocol PhotoCaptureViewControllerDelegate: NSObjectProtocol {
    ...
    func returnAssets(_ controller: PhotoCaptureViewController) -> [Asset]
...
open class PhotoCaptureViewController: UIViewController, PhotoCollectionViewLayoutDelegate {
    ...
    @objc func doneButtonTapped(_: UIButton) {
        let monTab:[Asset]
        var imagess:[UIImage] = []
        monTab = (delegate?.returnAssets(self))!
        for(index,item) in monTab.enumerated(){
        let asset = monTab[index]
        asset.originalImage({ image in
            imagess.append(image)
            if(imagess.count == monTab.count){
                print("dd \(imagess)")
